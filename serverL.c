...
void process_request(struct library *booklist){
    ...
    if(client_code[0] == 'X'){
        priority = 1;
        client_code[0] = 'L';
        printf("ServerL received an inventory status request for code %s.\n", client_code);
        ...
        printf("ServerL received %s code from the Main Server.\n", client_code);
        for(int i = 0; i < booklist->num_book; i++){
            if(strcmp(client_code, booklist->book_name[i]) == 0 && (0 < booklist->book_count[i])){
                availability = -1;
    ...
    if(priority == 1){
        printf("ServerL finished sending the  inventory status to the Main server using UDP on port %s.\n", MYSERVERPORT);
    }
    else{
        printf("ServerL finished sending the availability status of code %s to the Main Server using UDP on port %s.\n", client_code, MYSERVERPORT);    
    }
...
int main()
{
    FILE* ptr = fopen("literature.txt", "r");
    if (ptr == NULL) {
        printf("no such file: literature.txt");
        return 0;
    }
